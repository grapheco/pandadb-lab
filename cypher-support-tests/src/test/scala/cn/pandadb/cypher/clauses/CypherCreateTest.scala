...
class CypherCreateTest {
  ...
  def node_DateTime2(): Unit = {
    graphFacade.cypher("CREATE (Keanu:Person {name:'node_Date2',born:datetime('2018-02-03T12:09:11Z')})").show()
    val res = graphFacade.cypher("MATCH (Keanu:Person {name:'node_Date2'}) RETURN Keanu.born").records()
    Assert.assertEquals(ZonedDateTime.parse("2018-02-03T12:09:11Z"), res.next().get("Keanu.born").get.asInstanceOf[LynxDateTime].value)
  }
