...
object LdbcTestCypherAllTest4 {
  var nodeStore: NodeStoreSPI = _
  var relationStore: RelationStoreSPI = _
  var indexStore: IndexStoreAPI = _
  var statistics: Statistics = _
  var graphFacade: GraphFacade = _
  var cypherTemplates = ArrayBuffer[String]()

  def main(args: Array[String]): Unit = {
//    val args = Array("testdata/db2", "testdata/cyphers1.txt" ,"testdata/args.txt")
    if(args.length < 1) {
      println("需要数据路径")
      return
    }
    ...
    if(args.length > 1) {
      val cypherPath: String = args(1)
      println(s"cypher文件路径：${cypherPath}")
      try {
        val br = new BufferedReader(new InputStreamReader(new FileInputStream(cypherPath)))
        br.lines().forEach(line=>cypherTemplates.append(line.trim()))
        br.close() //关闭IO
      } catch {
        case e: Exception =>
    ...
    if(args.length > 2) {
      ...
      println("未提供参数文件路径")
      for(tmp <- 0 until cypherTemplates.length) {
        cyphersArgs.append(Array[String]("x"))
      }
    }

    assert(cyphersArgs.length == cypherTemplates.length)
  ...
  def setup(dbPath: String="./testdata"): Unit = {
    println(s"数据路径：${dbPath}")
    nodeStore = new CachedNodeStoreAPI(dbPath)
  ...
  def LDBC(cyphersArgs: Array[Array[String]]): Unit ={
    ...
    assert(cypherTemplates.length == cyphersArgs.length)
    ...
    for (i <- 0 until cypherTemplates.length) {
      println(s"---- cypher $i ----")
      val cypherTpl = cypherTemplates(i)
...
//  val cypherTemplates = Array[String](
